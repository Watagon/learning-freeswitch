--[[
  sending tiff image with fax
]]

-- 読み込む画像
local fax_file_path = "/tmp/artifacts/sample.tiff"

-- スクリプト開始をログに出力
freeswitch.consoleLog("INFO", "== FAX送信スクリプト開始: " .. session:getVariable("uuid") .. " ==\n")

-- セッションに応答
session:answer()

-- txfaxアプリケーションを実行してFAXを送信
-- 第一引数: 保存先のファイルパス
freeswitch.consoleLog("NOTICE", "FAXの送信を開始します。元データ: " .. fax_file_path .. "\n")
session:execute("txfax", fax_file_path)
...
if fax_success == "1" then
  ...
  freeswitch.consoleLog("NOTICE", "FAXの送信に成功しました。\n")
  freeswitch.consoleLog("NOTICE", "  - ファイルパス: " .. fax_file_path .. "\n")
  freeswitch.consoleLog("NOTICE", "  - 送信先ID: " .. remote_station_id .. "\n")
  ...
  freeswitch.consoleLog("WARNING", "FAXの送信に失敗しました。\n")
  freeswitch.consoleLog("WARNING", "  - 理由: " .. fax_result_text .. " (Code: " .. fax_result_code .. ")\n")
end

-- スクリプト終了をログに出力
freeswitch.consoleLog("INFO", "== FAX送信スクリプト終了: " .. session:getVariable("uuid") .. " ==\n")
